package com.recursion;

public class PowerProblem
{
    public static long modfun(long n, long  R)
    {
        // Base cases 
        if (n == 0) 
            return 0; 
        if (R == 0)  //power zero mean answer is 1
            return 1; 
      
        // If R is even 
        long y; 
        if (R % 2 == 0) { 
            y = modfun(n, R/2);  //finding r/2 power as power is even then storing answer in y and---
            y = (y * y); //---if power is even like 2^4 we can simply do (2^2)*(2^2)
        } 
      
        // If R is odd 
        else { 
            y = n * modfun(n, R - 1) ; //--- reduce the power by 1 to make it even. The reducing power
            //by one can be done if we take one n out. Like 2^3 can be written as 2*(2^2)
        } 
      
        return y % 1000000007;  
        //finally return the answer (y+mod)%mod = (y%mod+mod%mod)%mod = (y%mod)%mod
    }
    /*
     *Time complexity->O(R) 
     */
    long power(int N,int R)
    {
        //Your code here
        if(R == 0){
            return 1;
        }
        return (N*power(N,R-1))%1000000007;
    }
    
	public static void main(String[] args) {
		int number = 12;
		int reverse = 21;
		System.out.println(modfun(number,reverse));
	}

}
